...
using ExternalCounterstrike.Overlay.Structs;
...
namespace ExternalCounterstrike.Overlay
{
    internal class Direct2DRenderer
    {
        ...
        private static readonly RawColor4 Transparent = new RawColor4(Color.Transparent.R, Color.Transparent.G, Color.Transparent.B, Color.Transparent.A);
        ...
        {
            _brushContainer.Add(new SolidColorBrush(_device, new RawColor4(color >> 16 & 255L, color >> 8 & 255L, (byte)color & 255L, color >> 24 & 255L)));
        ...
        public void DrawHakenkreuz(int x, int y, int size, float stroke, int brush)
        {
            //left to right
            ...
            //haken
            var haken1 = new RawVector2(third.X + size, third.Y);
            var haken2 = new RawVector2(second.X, second.Y + size);
            var haken3 = new RawVector2(fourth.X - size, fourth.Y);
            var haken4 = new RawVector2(first.X, first.Y - size);
        ...
        {
            _device.DrawRectangle(new RawRectangleF(x, y, x + width, y + height), _brushContainer[brush], stroke);
            _device.FillRectangle(new RawRectangleF(x + stroke, y + stroke, x + width - stroke, y + height - stroke), _brushContainer[interiorBrush]);
        }
        ...
        public void BorderedLine(int startX, int startY, int endX, int endY, float stroke, int brush, int borderBrush)
        {
            ...
            _device.DrawLine(new RawVector2(startX, startY - stroke), new RawVector2(endX, endY - stroke), _brushContainer[borderBrush], stroke); //top
            _device.DrawLine(new RawVector2(startX, startY + stroke), new RawVector2(endX, endY + stroke), _brushContainer[borderBrush], stroke); //bot

            _device.DrawLine(new RawVector2(startX - stroke / 2, startY - stroke * 1.5f), new RawVector2(startX - stroke / 2, startY + stroke * 1.5f), _brushContainer[borderBrush], stroke);//left
            _device.DrawLine(new RawVector2(endX - stroke / 2, endY - stroke * 1.5f), new RawVector2(endX - stroke / 2, endY + stroke * 1.5f), _brushContainer[borderBrush], stroke);///rigth
        }
        ...
        {
            _device.DrawRectangle(new RawRectangleF(x - (stroke - borderStroke), y - (stroke - borderStroke), x + width + stroke - borderStroke, y + height + stroke - borderStroke), _brushContainer[borderBrush], borderStroke);
        ...
        {
            _device.DrawEllipse(new Ellipse(new RawVector2(x, y), radius + stroke, radius + stroke), _brushContainer[borderBrush], stroke);
